 ...
 * If `model` is omitted, we use the real default model below (prod-safe).
 */
async function getLLMResponse({
  ...
  console.log('[AI] response', {
    ...
    contentPreview: preview(content, 180),
  });

  if (!trimmed) {
    console.warn('[AI] WARNING: LLM returned empty content', { model });
  }
...
export async function extractMetadataFromText(text) {
  ...
  console.log('[AI] extractMetadata raw response preview', { preview: preview(aiResponseText, 200) });
